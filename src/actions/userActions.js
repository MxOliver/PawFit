...
function saveProfile(data, userId) {
	return dispatch => {
		...
		userService.addProfile(data, userId).then(
			profile => {
	...
	function request(data) {
		return { type: userConstants.ADD_PROFILE_REQUEST, data };
	}
	function success(profile) {
		return { type: userConstants.ADD_PROFILE_SUCCESS, profile };
	}
	function failure(error) {
		return { type: userConstants.ADD_PROFILE_FAILURE, error };
	}
}
...
function getProfile(userId) {
	return dispatch => {
		...
		userService.fetchProfile(userId).then(
			profile => {
				dispatch(success(profile));
			},
			error => {
	...
	function request(userId) {
		return { type: userConstants.GET_PROFILE_REQUEST, userId };
	}
	function success(profile) {
